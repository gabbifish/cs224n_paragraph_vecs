	...
	for line in test_articles:
		...
		curr_cat = line[2:].replace('\n', '') # get topic name
		    ...
		    filename = file.replace('http://en.wikipedia.org/wiki/', '') + '.txt'
                    ...
                    #UPDATE'Dbow+dmc' Concat models 2,1
                    model_1_vec = self.model1.infer_vector(text)
                    model_2_vec = self.model2.infer_vector(text) 
                    wiki_entry_matrix[article_count,] = np.concatenate([model_2_vec, model_1_vec])
