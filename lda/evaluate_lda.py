...
#model = gensim.models.Word2Vec(train_sentences, sg=0,hs=1, min_count=1, size=dimension)
#train the model? 
...
def getFeatureVec(dictionary, reviews, model, review_index):
    # Given a set of reviews (each one a list of words), calculate 
    # the average feature vector for each one and return a 2D numpy array 
    #

    #reviews seems to be correct format...
    # Loop through the reviewsdimensio
...
#
  ...
  # print type(counter)
    print len(doc)
        #reviewFeatureVecs[int(counter)] = doc
#makeFeatureVec(review, model, num_features)
       #
...
print "Creating document feature vecs for test articles..."
...
for i in range(0, num_test_triplets):
    ...
    dense2 = gensim.matutils.sparse2full(getFeatureVec(dictionary, clean_test_articles, model, article_2_index), model.num_topics)
    dense3 = gensim.matutils.sparse2full(getFeatureVec(dictionary, clean_test_articles, model, article_3_index), model.num_topics)        
    hell12 = np.sqrt(0.5 * ((np.sqrt(dense1) - np.sqrt(dense2))**2).sum())
    hell23 = np.sqrt(0.5 * ((np.sqrt(dense2) - np.sqrt(dense3))**2).sum())
    if abs(hell12) < abs(hell23):
        correct_count += 1 
